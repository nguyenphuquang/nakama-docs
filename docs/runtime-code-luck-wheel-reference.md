...
Authenticate user and create a session token using a custom authentication managed by an external service or source not already supported by Itme-platform. Best suited for use with existing external identity services.
...
Generate a Itme-platform session token from a username. This is not the same as an authentication mechanism because a user does not get created and input is not checked against the database.
...
!!! Note
    These functions allow your Lua scripts to run arbitrary SQL staments beyond the ones built into Itme-platform itself. It is your responsibility to manage the performance of these queries.
